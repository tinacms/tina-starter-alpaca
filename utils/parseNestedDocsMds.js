...
const parseNestiness = (key, list, document) => {
  ...
  if (foundIndex !== -1) {
    ...
    listCopy.push(
      parseParentItem(key, document, 'link')
    )
  }
  return listCopy
}
...
const parseGroupChildren = (key, childrenList, document) => {
  ...
  if (foundIndex !== -1) {
    childrenListCopy[foundIndex].children.push(
      parseChildItem(key, document)
    )
  } else {
    childrenListCopy.push(
      parseParentItem(key, document, 'group')
    )
  }
